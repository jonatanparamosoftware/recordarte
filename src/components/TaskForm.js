    ...
    render () {
        return (
            <form onSubmit={this.onSubmit}> {/*onsubmit es para que no se refresque la pagina cada que le damos enviar */}
                <input type="text" placeholder="Write a Task" onChange={this.onChange}/> {/*placeholder sirve para agregar un campo en el cual se puede escribir */}
                <br/>
                <br/>
                <textarea placeholder="Write a Description" onChange={this.onChange}></textarea> {/*onchance  es para que se escuche lo que se escriba en los dos input, y se escucha por medio de e.target.value*/}
                <input type="submit"/>
            </form>
        )
    }
